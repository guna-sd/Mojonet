...
# from mc10.DataPointer import DataPointer
...
# ===----------------------------------------------------------------------=== #
# Comprehensive Tests for DataPointer and UniquePointer
# ===----------------------------------------------------------------------=== #

from testing import assert_equal, assert_true, assert_false, TestSuite
from mc10.hardware.device import Device
from builtin.type_aliases import MutOrigin
from mc10.utils.unique_ptr import UniquePointer
from mc10.core.pointer import DataPointer
...
fn test_unique_pointer_default_construction() raises:
    """Test default construction creates null pointer."""
    var ptr = UniquePointer[Int]()
    assert_false(ptr)


fn test_unique_pointer_construct_with_none() raises:
    """Test construction with None creates null pointer."""
    var ptr = UniquePointer[Int](None)
    assert_false(ptr)


fn test_unique_pointer_bool_conversion() raises:
    """Test bool conversion of null UniquePointer."""
    var null_ptr = UniquePointer[Int]()
    assert_false(null_ptr)


fn test_unique_pointer_equality_none() raises:
    """Test equality comparison with None."""
    var ptr = UniquePointer[Int]()
    assert_true(ptr == None)
    assert_false(ptr != None)


fn test_unique_pointer_get_null() raises:
    """Test get method on null pointer."""
    var ptr = UniquePointer[Int]()
    var raw = ptr.get()
    assert_equal(raw, UnsafePointer[Int, MutOrigin.external]())


fn test_unique_pointer_clear() raises:
    """Test clear method resets pointer."""
    var ptr = UniquePointer[Int]()
    assert_false(ptr)
    ptr.clear()
    assert_false(ptr)
...
fn test_unique_pointer_release_null() raises:
    """Test release method on null pointer."""
    var ptr = UniquePointer[Int]()
    var released = ptr.release()
    assert_false(ptr)
    assert_equal(released, UnsafePointer[Int, MutOrigin.external]())


fn test_unique_pointer_reset_null() raises:
    """Test reset method with null pointer."""
    var ptr = UniquePointer[Int]()
    var new_ptr = UnsafePointer[Int, MutOrigin.external]()
    ptr.reset(new_ptr)
    assert_equal(ptr.get(), new_ptr)


fn test_unique_pointer_swap_nulls() raises:
    """Test swap exchanges null pointers."""
    var ptr1 = UniquePointer[Int]()
    var ptr2 = UniquePointer[Int]()
...
fn test_unique_pointer_get_deleter() raises:
    """Test get_deleter returns deleter function."""
    var ptr = UniquePointer[Int]()
    var deleter = ptr.get_deleter()
    # Should successfully get deleter
    _ = deleter
...
fn test_unique_pointer_different_types_uint() raises:
    """Test UniquePointer with unsigned int type."""
    var ptr = UniquePointer[UInt]()
    assert_false(ptr)


fn test_unique_pointer_different_types_float() raises:
    """Test UniquePointer with float type."""
    var ptr = UniquePointer[Float32]()
    assert_false(ptr)


fn test_unique_pointer_different_types_string() raises:
    """Test UniquePointer with string type."""
    var ptr = UniquePointer[String]()
    assert_false(ptr)


# ===----------------------------------------------------------------------=== #
# DataPointer Tests - Initialization and Device
# ===----------------------------------------------------------------------=== #


fn test_data_pointer_default_construction() raises:
    """Test default construction creates null DataPointer."""
    var ptr = DataPointer()
    assert_false(ptr)
    assert_equal(ptr.device(), Device.CPU)


fn test_data_pointer_bool_conversion() raises:
    """Test bool conversion of null pointer."""
    var ptr = DataPointer()
    assert_false(ptr)


fn test_data_pointer_equality_none() raises:
...
# ===----------------------------------------------------------------------=== #
# DataPointer Tests - Operations
# ===----------------------------------------------------------------------=== #


fn test_data_pointer_clear() raises:
    """Test clear resets pointer."""
    var ptr = DataPointer()
    assert_false(ptr)
    ptr.clear()
    assert_false(ptr)
...
fn test_data_pointer_get_context() raises:
    """Test get_context returns opaque pointer."""
    var ptr = DataPointer()
    var ctx = ptr.get_context()
    assert_equal(ctx, OpaquePointer[MutOrigin.external]())


fn test_data_pointer_release_context() raises:
    """Test release_context."""
    var ptr = DataPointer()
    var released = ptr.release_context()
    assert_equal(released, OpaquePointer[MutOrigin.external]())


fn test_data_pointer_unsafe_set_device() raises:
    """Test unsafe_set_device changes device."""
    var ptr = DataPointer()
    assert_equal(ptr.device(), Device.CPU)

    ptr.unsafe_set_device(Device.CPU)
    assert_equal(ptr.device(), Device.CPU)


# ===----------------------------------------------------------------------=== #
# DataPointer Tests - Move Semantics
# ===----------------------------------------------------------------------=== #


fn test_data_pointer_move_constructor() raises:
    """Test move constructor transfers state."""
    var ptr1 = DataPointer()
    var device1 = ptr1.device()

    var ptr2 = ptr1^
    assert_equal(ptr2.device(), device1)


fn test_data_pointer_move_context() raises:
    """Test move_context transfers ownership."""
    var ptr = DataPointer(alloc[NoneType](1))
    var ctx = ptr.move_context()
    _ = ctx
    assert_false(ptr)


# ===----------------------------------------------------------------------=== #
# DataPointer Tests - Consistency
# ===----------------------------------------------------------------------=== #


fn test_data_pointer_default_device_is_cpu() raises:
    """Test that default device is CPU."""
    var ptr = DataPointer()
    assert_equal(ptr.device(), Device.CPU)


fn test_data_pointer_device_consistency() raises:
    """Test device consistency after operations."""
    var ptr = DataPointer(Device.CPU)
...
fn test_unique_pointer_chain_operations() raises:
    """Test multiple sequential operations on UniquePointer."""
    var ptr = UniquePointer[Int]()
    assert_false(ptr)
...
fn test_data_pointer_chain_operations() raises:
    """Test multiple sequential operations on DataPointer."""
    var ptr = DataPointer()
    ...
    var raw = ptr.get()
    assert_equal(raw, OpaquePointer[MutOrigin.external]())
